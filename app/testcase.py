...
class Testcase(Element):
    ...
    def execute_case(self):
        ...
        if self.action in ['click','sendkey','swipe','swipe2','back','tag','tap','sleep']:
            # 获取页面元素
            try:
            ...
            if self.action == 'click':
                ...
                if '.' in str(self.value):
                    element.send_keys(unicode(str(self.value).split('.')[0]))
                else:
                    element.send_keys (unicode(str (self.value)))
                ...
                if not self.Swipe (self.driver, str(swipe_value[0]), float(swipe_value[1]), int(swipe_value[2])):
                    print('滑动失败')
                # if len (swipe_value) == 3:
                #     if self.Swipe (self.driver, str(swipe_value[0]), float(swipe_value[1]), int(swipe_value[2])):
                #         print('滑动成功')
                #     else:
                #         print('滑动失败')
                # else:
                #     if self.Swipe (self.driver, str (swipe_value[0]), float (swipe_value[1])):
                #         print('滑动成功')
                #     else:
                #         print('滑动失败')
                ...
                if self.Tag(self.driver,self.value):
                    ...
                    print("按住坐标失败")
            elif self.action == 'tap':
                if self.Tap (self.driver, self.value):
                    print("点击坐标成功")
                else:
                    print("点击坐标失败")
